...
const FormProduct = ({ form, setForm, update, queryClient, options }) => {
	...
	const onSubmit = async (data) => {
		data.price_buy = data.price_buy.replace(/[$]/g, "")
		data.price_sell = data.price_sell.replace(/[$]/g, "")
		data.price_buy = parseFloat(data.price_buy.replace(".,", ""))
		data.price_sell = parseFloat(data.price_sell.replace(".,", ""))
		if (isUpdate) {
			const id = row.id;
						...
						<div className={styles.inputs}>
							...
							<div className={styles.input}>
								...
								{errors.id_category?.message && (
									<ErrorMessage message={errors.id_category.message} />
								)}
							</div>
							<div className={styles.input}>
								<NumberField
